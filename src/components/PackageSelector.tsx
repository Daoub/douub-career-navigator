...
const PackageSelector: React.FC = () => {
  ...
  return <div className="space-y-8" dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {packages.map(pkg => <Card key={pkg.id} className={`relative overflow-hidden transition-all duration-300 cursor-pointer ${selectedPackage === pkg.id ? 'ring-2 ring-blue-500 shadow-xl scale-105' : 'hover:shadow-lg hover:scale-102'} ${pkg.popular ? 'border-blue-500' : ''} ${pkg.premium ? 'border-purple-500' : ''}`} onClick={() => handleSelectPackage(pkg.id)}>
            {/* Popular Badge */}
            {pkg.popular && <div className="absolute top-4 left-4 z-10">
                <Badge className="bg-blue-500 text-white">الأكثر شعبية</Badge>
              </div>}
            
            {/* Premium Badge */}
            {pkg.premium && <div className="absolute top-4 left-4 z-10">
      ...
      <Card className="bg-gradient-to-br from-slate-50 to-blue-50">
        <CardContent className="p-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <Button onClick={handlePurchase} size="lg" className="bg-gradient-to-r from-emerald-600 to-blue-600 hover:from-emerald-700 hover:to-blue-700 text-white px-8">
              إتمام الشراء
            </Button>
